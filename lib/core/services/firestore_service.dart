...
class FirestoreService {
  ...
  CollectionReference<Map<String, dynamic>> get _pendingSchedules =>
      _db.collection('pending_schedules');

  CollectionReference<Map<String, dynamic>> get _liveSchedules =>
      _db.collection('live_schedules');

  CollectionReference<Map<String, dynamic>> get _notifications =>
      _db.collection('notifications');

  /// Stream of a single pending schedule document for [date] (YYYY-MM-DD).
  Stream<DocumentSnapshot<Map<String, dynamic>>> getPendingSchedule(
      String date) {
    return _pendingSchedules.doc(date).snapshots();
  }

  /// Stream of a single live schedule document for [date] (YYYY-MM-DD).
  Stream<DocumentSnapshot<Map<String, dynamic>>> getLiveSchedule(String date) {
    return _liveSchedules.doc(date).snapshots();
  }

  /// One-shot read of the live schedule for [date] — used by chatbot context.
  Future<Map<String, dynamic>?> getLiveScheduleOnce(String date) async {
    final doc = await _liveSchedules.doc(date).get();
  ...
  /// Admin approval: copy pending → live, update status, delete pending.
  Future<void> approvePendingSchedule(String date) async {
    final pendingDoc = await _pendingSchedules.doc(date).get();
    if (!pendingDoc.exists) throw Exception('No pending schedule for $date');

    final data = Map<String, dynamic>.from(pendingDoc.data()!);
    data['status'] = 'approved';
    data['approvedAt'] = DateTime.now();

    await _db.runTransaction((txn) async {
      txn.set(_liveSchedules.doc(date), data);
      txn.delete(_pendingSchedules.doc(date));
    });

    // Update the matching notification document status.
    final notifQuery = await _notifications
        .where('date', isEqualTo: date)
        .where('type', isEqualTo: 'schedule_update')
        .limit(1)
        .get();
    if (notifQuery.docs.isNotEmpty) {
      await notifQuery.docs.first.reference.update({'status': 'approved'});
    }
  }

  /// Admin rejection: delete pending, update notification status.
  Future<void> rejectPendingSchedule(String date) async {
    await _pendingSchedules.doc(date).delete();
    ...
    final notifQuery = await _notifications
        ...
        .get();
    if (notifQuery.docs.isNotEmpty) {
      await notifQuery.docs.first.reference.update({'status': 'rejected'});
    }
  }

  // ── Notifications ──────────────────────────────────────────────────────────

  /// Stream of all notification documents ordered newest-first.
  ...
  /// Write a notification document (from Cloud Function or approval flow).
  Future<void> writeNotification(Map<String, dynamic> data) {
